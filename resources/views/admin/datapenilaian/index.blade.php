                            ...
                            <tbody class="bg-white text-gray-800">
                                @foreach ($gurus as $index => $guru)
                                    ...
                                    <tr
                                        ...
                                        <td class="border px-4 py-2">
                                            {{ $guru->penilaianAdmin->administrasiSubKriteria->nama_sub_kriteria ?? '-' }}
                                        </td>
                                        <td class="border px-4 py-2">
                                            {{ $guru->penilaianAdmin->presensi_realita ?? '-' }}</td>
                                        <td class="border px-4 py-2">
                                            {{ $guru->penilaianAdmin->sertifikat_pengembangan ?? '-' }}</td>
                                        <td class="border px-4 py-2">{{ $guru->penilaianAdmin->kegiatan_sosial ?? '-' }}
                                        </td>
                                    ...
                                    <div id="modal{{ $guru->id }}"
                                        class="fixed inset-0 bg-gray-800 bg-opacity-50 hidden flex items-center justify-center z-50">
                                        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                                            <h2 class="text-xl font-semibold mb-4 text-center">
                                                {{ $guru->penilaianAdmin ? 'Edit' : 'Tambah' }} Penilaian -
                                                {{ $guru->user->name ?? '-' }}</h2>

                                            <form method="POST"
                                                action="{{ $guru->penilaianAdmin ? route('admin.datapenilaian.update', $guru->penilaianAdmin->id) : route('admin.datapenilaian.store') }}">
                                                @csrf
                                                @if ($guru->penilaianAdmin)
                                                    @method('PUT')
                                                @endif
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <select name="administrasi" id="administrasi"
                                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                                        required>
                                                        <option value="">-- Pilih Sub Kriteria --</option>
                                                        @foreach ($subkriteriaAdministrasi as $sub)
                                                            <option value="{{ $sub->id_sub_kriteria }}"
                                                                {{ isset($guru->penilaianAdmin) && $guru->penilaianAdmin->administrasi == $sub->id_sub_kriteria ? 'selected' : '' }}>
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="number" step="0.01" name="presensi_realita"
                                                        id="presensi_realita"
                                                ...
                                                <div class="mb-4">
                                                    <label for="sertifikat_pengembangan"
                                                        class="block text-sm font-medium text-gray-700">Sertifikat
                                                        Pengembangan</label>
                                                    <select name="sertifikat_pengembangan" id="sertifikat_pengembangan"
                                                        ...
                                                        @for ($i = 0; $i <= 2; $i++)
                                                            <option value="{{ $i }}"
                                                                {{ isset($guru->penilaianAdmin) && $guru->penilaianAdmin->sertifikat_pengembangan == $i ? 'selected' : '' }}>
                                                                {{ $i }}
                                                            </option>
                                                        @endfor
                                                    </select>
                                                ...
                                                <div class="mb-4">
                                                    <label for="kegiatan_sosial"
                                                        class="block text-sm font-medium text-gray-700">Kegiatan
                                                        Sosial</label>
                                                    <select name="kegiatan_sosial" id="kegiatan_sosial"
                                                        ...
                                                        @for ($i = 0; $i <= 2; $i++)
                                                            <option value="{{ $i }}"
                                                                {{ isset($guru->penilaianAdmin) && $guru->penilaianAdmin->kegiatan_sosial == $i ? 'selected' : '' }}>
                                                                {{ $i }}
                                                            </option>
                                                        @endfor
                                                    </select>
                                                ...
                                                <div class="flex justify-end">
                                                    <button type="button"
                                                        onclick="document.getElementById('modal{{ $guru->id }}').classList.add('hidden')"
                                            ...
                                            <form method="POST"
                                                ...
                                                <div class="mb-4">
                                                    <label for="sertifikat_pengembangan"
                                                        class="block text-sm font-medium text-gray-700">Sertifikat
                                                        Pengembangan</label>
                                                    <select name="sertifikat_pengembangan"
                                                        id="sertifikat_pengembangan"
                                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                                        required>
                                                        <option value="">-- Pilih Nilai --</option>
                                                        @for ($i = 0; $i <= 2; $i++)
